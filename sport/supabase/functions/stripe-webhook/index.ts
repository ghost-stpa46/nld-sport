      ...
      if (devisData) {
        ...
        if (brevoKey) {
          await sendEmail(brevoKey, 'nolackindiscipline@gmail.com',
            `Nouveau client payant — ${devisData.prospect_prenom} ${devisData.prospect_nom}`,
            ...
            <hr/>
            <p>Le client peut maintenant créer son compte.</p>`
          );

          await sendEmail(brevoKey, devisData.prospect_email,
            `Bienvenue chez noLackinDiscipline — Crée ton compte`,
            `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#0a0a0a;color:#ffffff;padding:40px 32px;border-radius:12px;">
              ...
              <p style="color:#ccc;line-height:1.6;margin:0 0 32px">
                Ton paiement de <strong style="color:#fff">${devisData.montant} €</strong> a bien été reçu.<br/>
                Crée ton compte pour accéder à ton espace client.
              </p>
